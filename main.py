import telebot
import os  # Убедись, что os импортирован, он нам нужен
...
# Берем токен из переменных окружения, которые мы настроили на сервере
TOKEN = os.getenv('TELEGRAM_BOT_TOKEN')

# --- ЖЕСТКАЯ ОТЛАДОЧНАЯ ПРОВЕРКА ---
print("="*40)
print("--- НАЧАЛО ОТЛАДКИ ---")
print(f"Скрипт пытается использовать токен: '{TOKEN}'")
print("--- КОНЕЦ ОТЛАДКИ ---")
print("="*40)
# --- КОНЕЦ ПРОВЕРКИ ---

# Проверка на случай, если ты забыл добавить токен на сервере
if not TOKEN:
    print("\n!!! ОШИБКА: Токен пустой (None). Скрипт не видит переменную окружения. Проверь команду 'export' !!!")
    exit() # Выходим из скрипта, чтобы не было ошибки 401
